...
class OpenAIBatchRunner:
    ...
    def create_jsonl_batches(self, batch_size: int = 500):
        ...
        # Flatten the grouped structure into a list of individual reviews with traceability
        flattened_reviews = []
        for asin_entry in reviews_data:
            asin = asin_entry["asin"]
            for rating, reviews in asin_entry["ratings"].items():
        ...
        num_batches = len(flattened_reviews) // batch_size + (
            1 if len(flattened_reviews) % batch_size != 0 else 0
        )
        ...
        global_review_counter = 0  # Counter to ensure unique custom_id across all batches

        for batch_num in range(num_batches):
            batch_list = []
            start_idx, end_idx = batch_num * batch_size, (batch_num + 1) * batch_size
            batch_reviews = flattened_reviews[start_idx:end_idx]

            for review in batch_reviews:
                # Ensure unique custom_id using ASIN, rating, and review_id
                batch_json["custom_id"] = f"asin_{review['asin']}_rating_star_{review['rating']}"
